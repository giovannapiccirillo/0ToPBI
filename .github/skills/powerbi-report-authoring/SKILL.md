...
description: >-
  ...
  design, use `powerbi-report-design` first. For requirements gathering and
  end-to-end phase orchestration, use `powerbi-requirements-gathering` and the
  `orchestrator` agent first. Triggers: "edit PBIR",
...
- Start from an approved `Design Brief:` (produced by `powerbi-report-design`) or approved `output/requirements.md` for greenfield report builds.
...
- Do not use only this skill for open-ended design or requirements gathering; pair it with `powerbi-report-design` or `powerbi-requirements-gathering`.

## Quick Start Workflow

0. **Plan/design routing** → for greenfield builds, ensure requirements were
   gathered via `powerbi-requirements-gathering` (`output/requirements.md`)
   under the `orchestrator` flow first; for theming, visual style, layout,
   redesigns, or critiques, read `powerbi-report-design`. Return here for PBIR
   mechanics. Before authoring, use the `Design Brief:` yaml block produced by
   `powerbi-report-design` (or an approved inline `Design Brief:` block in the
   conversation) as implementation context.
...
>
> Use `powerbi-requirements-gathering` (via the `orchestrator` agent) before
> authoring for new report/dashboard requests, requirements gathering, or
> approval on `output/requirements.md`. Use `powerbi-report-design` for open-ended visual design,
