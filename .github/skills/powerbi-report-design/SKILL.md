...
description: >-
  ...
  `powerbi-report-authoring`. For requirements gathering, use
  `powerbi-requirements-gathering`; for end-to-end orchestration across
  phases, use the `orchestrator` agent. Triggers: "design Power BI report",
...
When this skill is used inside the `report-builder` workflow (phase 3), embed
this YAML block in the report-builder's design output under a "Canonical
design contract" section, using `output/requirements.md` as the approved
input. For the full template, field rationale, brownfield current/target
fields, minimal-brief escape hatch, page-level mechanical schema, examples,
and validation checklist, read `references/design-brief.md`.
