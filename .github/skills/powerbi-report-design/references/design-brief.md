...
In the `report-builder` workflow (phase 3), embed this YAML block in the
design output under a "Canonical design contract" section, starting from the
approved `output/requirements.md`. The surrounding Markdown is for user
approval and context; the embedded YAML is authoritative for implementation.
