...
description: >
  ...
  Supports both Power BI Desktop and Fabric Service development workflows.
...
| Determine which relationships the required measures need (relationships.yaml)   | [Propose Relationships from Requirements](#workflow-propose-relationships-from-requirements) |
...
| Relationship Proposal YAML       | [relationship-proposal.md](./references/relationship-proposal.md)                  | Before writing or consuming `output/relationships.yaml`                                     |
...
2. **Tier 2 — MCP not registered + PBIP folder or Fabric workspace** -> **In this project, never hand-author TMDL**: fill the declarative spec `output/model.yaml` and run `python scripts/04_modello/build_model.py`, which generates the whole `definition/` folder (correct tab indentation, structure, UTF-8, partitions, relationships from `output/relationships.yaml`) and runs the phase-2 validation gate automatically. Hand-written TMDL has repeatedly produced flat (non-indented) files that Power BI Desktop rejects with a TMDL Indentation error. Load [tmdl-guidelines.md](./references/tmdl-guidelines.md) and [pbip.md](./references/pbip.md) only to understand the format, not to write it by hand. When the source is a Fabric workspace, use `az rest` to round-trip the TMDL (load [semantic-model-rest-api.md](./references/semantic-model-rest-api.md)): `getDefinition` -> edit TMDL locally -> `updateDefinition`.
...
- **Fabric workspace**: First, find the workspace and semantic model: list workspaces to resolve the workspace ID by name, then list items of type `SemanticModel` in that workspace to resolve the model ID by name. Then connect to the model (live) or export its TMDL definition for local editing.
...
3. **Design star schema** - identify fact and dimension tables and relationship keys.
   - If the build serves an approved requirements doc and involves multiple source tables, run [Propose Relationships from Requirements](#workflow-propose-relationships-from-requirements) first: relationships are then created only from the approved `output/relationships.yaml` (or skipped entirely if that workflow concluded none are needed).
...
## Workflow: Propose Relationships from Requirements

**When this applies:** Before creating the measures required by an approved requirements document (`output/requirements.md`), when the model is built from multiple source tables — or whenever it must be decided whether relationships are needed at all. In this project this is Step 1 of the semantic-modeler agent (phase 2), executed **before** any measure is written.

Load [relationship-proposal.md](./references/relationship-proposal.md) for the YAML schema and validation rules before writing the file.

Steps:

1. **Map measures to tables** - read the required measures/KPIs from `output/requirements.md` and the available tables (`staging/<Project>/`, falling back to `input/`). For each measure, list the tables and columns it needs.
2. **Decide** - exactly one of:
   - **All measures resolve from a single table** -> no relationships are needed. State this in one line, do NOT create `output/relationships.yaml`, and proceed directly to the measure-creation step. Do not propose relationships "for completeness".
   - **At least one measure spans multiple tables** -> continue with step 3.
3. **Propose** - for each needed relationship, present to the user: many-side table/column, one-side table/column, cardinality, cross-filter direction, active flag, and — crucially — which measure(s) from the requirements doc make it necessary. Follow [modeling-guidelines.md](./references/modeling-guidelines.md) defaults (many-to-one, single direction, star schema).
4. **Write the YAML on approval** - after explicit user approval, write `output/relationships.yaml` per the schema in [relationship-proposal.md](./references/relationship-proposal.md), and run its validation rules mechanically (every relationship has `required_by`, every cross-table measure is covered, keys exist with compatible types).
5. **Hand off to the build** - the model build ([Create new Semantic Model](#workflow-create-new-semantic-model) or [Modify an Existing Model](#workflow-modify-an-existing-model)) creates relationships **exclusively** from this YAML via the active tool tier (MCP `relationship_operations` in Tier 1) — one call per entry, no additions or changes at build time. A relationship discovered later re-enters this workflow (propose -> approve -> update YAML) before being created.

> **Note:** `output/relationships.yaml` is a design artifact, not model source code. Writing it never replaces creating the relationships through the [Tool Selection Priority](#tool-selection-priority) tools, and it must never be reverse-engineered from an already-built model to justify relationships after the fact.
...
4. **Execute** in correct order:
   ...
   - **Adding relationships** - ensure key columns exist on both sides with matching data types; if an approved `output/relationships.yaml` exists for this model, create exactly the relationships it lists (see [Propose Relationships from Requirements](#workflow-propose-relationships-from-requirements)) — new ones go through that workflow first;
