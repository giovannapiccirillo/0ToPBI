...
def phase_16_status(staging_dir):
    csvs = sorted(p.name for p in staging_dir.glob("*.csv"))
    log = staging_dir / "etl-log.md"
    if not csvs:
        return ["Cartella staging presente ma senza CSV: fase 1.6 NON conclusa."]
    out = [f"CSV in staging: {', '.join(csvs)}"]
    if not log.exists():
        out.append("etl-log.md ASSENTE: fase 1.6 NON conclusa (il log è output obbligatorio).")
        return out
    ...
    if missing:
        out.append("etl-log.md presente ma SENZA voce per: " + ", ".join(missing)
                   + " — fase 1.6 NON conclusa.")
    else:
        out.append("etl-log.md completo: una voce per ogni CSV — fase 1.6 conclusa (evidenza).")
    return out
...
def project_status(workdir):
    ...
    # Fase 1.5 — Analisi Dati
    print(f"Fase 1.5 — Analisi dati (output/{workdir}/data-analysis.md)")
    data_analysis = output_dir / "data-analysis.md"
    if not data_analysis.exists():
        print(f"  output/{workdir}/data-analysis.md ASSENTE: fase 1.5 da avviare (data-analyst).")
    else:
        code, lines = run_gate("validate_data_analysis.py", workdir)
        print_gate("fase 1.5 (validate_data_analysis.py)", code, lines)

    # Fase 1.6 — ETL
    print(f"Fase 1.6 — Pulizia dati (output/{workdir}/staging/)")
    staging = output_dir / "staging"
    if not staging.exists():
        print("  Nessuna cartella staging trovata: fase 1.6 non eseguita "
        ...
        for line in phase_16_status(staging):
            print(f"  {line}")
    ...
    if not project:
        print("Fase 2/3/4: nome progetto PBIP non trovato in requirements.md — impossibile proseguire.")
        return

    # Fase 2 — Modello semantico
    print(f"Fase 2 — Modello semantico (report/{project}.SemanticModel/)")
    ...
    print(f"Fase 3 — Layout report (report/{project}.Report/)")
